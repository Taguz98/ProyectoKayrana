...
public class DBParalelos extends Paralelos {
    ...
    public ArrayList<Paralelos> cargarParalelosCarrera(int idCarrera) {
        //Aqui guardaremos todos los paralelos que encontremos de una carrera
        ArrayList<Paralelos> paralelos = new ArrayList();

        try {
            String sql = "SELECT nombre_paralelo, ciclo_paralelo, id_paralelo\n"
                    + "FROM public.\"Paralelos\"\n"
                    + "WHERE fk_carrera = " + idCarrera + ";";
            ...
            while (rs.next()) {
                ...
                //Aqui consultamos las jornadas de un paralelo
                DBJornadas jd = new DBJornadas();
                par.setJornadas(jd.cargaParalelosJornada(par.getId()));
